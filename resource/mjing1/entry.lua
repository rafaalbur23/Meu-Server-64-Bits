...
function after_create_entry(entry) 
    ...
    local EntryName = "Aurora Area 1"
    SetMapEntryEventName( entry, EntryName )
    map_name, posx, posy, tmap_name = GetMapEntryPosInfo(entry)
    Notice("Aurora Area Portal opened at ["..posx..","..posy.."] Winter Islands")

end
function after_destroy_entry_mjing1(entry)
    map_name, posx, posy, tmap_name = GetMapEntryPosInfo(entry) 
    Notice("Access to Aurora Area 1 entrance has disappeared!") 

end

function after_player_login_mjing1(entry, player_name)
    map_name, posx, posy, tmap_name = GetMapEntryPosInfo(entry)
    ChaNotice(player_name, "Winter Island  opened Portal to Aurora Area 1")

end

function check_can_enter_mjing1( role, copy_mgr )
...
function begin_enter_mjing1(role, copy_mgr) 
	...
	SystemNotice(role,"Entering [Aurora area 1]") 
