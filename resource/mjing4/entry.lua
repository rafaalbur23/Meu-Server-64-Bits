...
function after_create_entry(entry) 
    ...
    local EntryName = "Dark Area 2"
    SetMapEntryEventName( entry, EntryName )
    map_name, posx, posy, tmap_name = GetMapEntryPosInfo(entry)
    Notice("Dark Area 2 Portal opened at ["..posx..","..posy.."] Winter Islands")

end

function after_destroy_entry_mjing4(entry)
    map_name, posx, posy, tmap_name = GetMapEntryPosInfo(entry) 
    Notice("Access to Dark Area 2 entrance has disappeared!") 

end

function after_player_login_mjing4(entry, player_name)
    map_name, posx, posy, tmap_name = GetMapEntryPosInfo(entry)
    ChaNotice(player_name, "Winter Island  opened Portal to Dark Area 2")

end

function check_can_enter_mjing4( role, copy_mgr )
...
function begin_enter_mjing4(role, copy_mgr) 
	...
	SystemNotice(role,"Entering [Dark Area 2]") 
	MoveCity(role, "mjing4")
