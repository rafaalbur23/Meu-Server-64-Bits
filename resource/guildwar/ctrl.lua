...
--									--
--------------------------------------------------------------------------
print( "Loading Guildwar Ctrl.lua" )


--此文件中，凡是可能被多次执行的函数，函数名都要加上地图名前缀

function config(map)
    MapCanSavePos(map, guildwar)	-- 1 = Allow players to save respaw // 0 = Do not allow players to save respaw // guildwar = Player will respaw in their base
    MapCanPK(map, 1)			-- 1 = Turns on PK Mouse Sword Icon // 0 = Turns off PK Mouse Sword Icon ( Need to press CTRL key )
    MapCanGuild(map,0)			-- 1 = Allow players to join a guild // 0 = Do not allow players to join a guild
    MapCopyNum(map, 1)			-- 1 Copy of the map will be generated by GameServer
    --MapCopyStartType(map, 1)		-- 设置地图立即开始
    SingleMapCopyPlyNum(map, 100)	-- 100 Players are allowed to enter the map
    MapCanTeam(map , 1)			-- 1 = Allow players to create party // 0 = Do not allow players to create party
    MapType ( map , 5 )			-- 0 = PvP Off // 1 = PvP Off // 2 = PvP On ( Not allowed to kill Guild and Party Mates ) // 3 = PvP On ( Allowed to kill any player but not Party Mates ) // 4 = PvP On ( Not allowed to kill Guild and Party Mates )        
    RepatriateDie(map,0)		-- 地图内复活
    SetMapGuildWar(map,1)		-- 1 = Set map as Sacred War map
...
function init_entry(map)
    ...
    SetMapEntryTime(map, "2005/8/30/19/00", "0/4/0", "0/3/0", "0/3/0")	-- The first set of numbers is the starting point (Year/Month/Day/Hour/Minute), next is opening interval (Day/Hour/Minute), portal open duration (Day/Hour/Minute), and last is the map duration (Day/Hour/Minute)
...
--
--	if Num_SSL >= 1 then
--	   SystemNotice(role, "your body <make life and death>")	
--	else
...
local Cha = TurnToCha(role) 
  ...
  local map_copy =  GetChaMapCopy(role)
     ...
     if check_need_show == 1 then
	    ...
	    MapCopyNotice ( map_copy , "[A Navy member left Sacred War]" )

         elseif GetChaGuildID(role) > 100 and GetChaGuildID(role) <= 200 then
            count_haidao = count_haidao - 1
	    MapCopyNotice ( map_copy , "[A Pirate member left Sacred War]" )
        end
    end
...
function map_copy_run_guildwar( map_copy )
    ...
    local closetime = GUILDWARCLOSETIME
		 ...
		 if closetime == GUILDCLOSESHOW[i] then
			 local Notice_all = "Sacred War will close in " .. closetime .. " seconds! Check announcement for more details. Enjoy!"
 ...
 local hp_hd = GetChaAttr( haidaoSide_BaseRole , ATTR_HP ) -----------------海盗刷BOSS
	...
	if percent_hd < 0.3 and create_boss_hd == 0 then
		...
		create_boss_hd = 1  --关闭
		MapCopyNotice ( map_copy ,"The Patron Saint of Pirate: you see the true face under a dark!")
	end
 ...
 local hp_hj = GetChaAttr( haijunSide_BaseRole , ATTR_HP ) -----------------海军刷BOSS
	...
	if percent_hj < 0.3 and create_boss_hj == 0 then
		...
		create_boss_hj = 1  --关闭
		MapCopyNotice ( map_copy ,"The Patron Saint of Navy: like the statue down? My hand to ask the sword!")
	end
...
--		local GuildName_Blue = GetGuildName ( BlueSide_GuildID )
		...
		if CheckMonsterDead ( haijunSide_BaseRole ) == 1 then
		   if haidaoSide_JDNotice == 0 then
			...
			five_seconds = 1
			--local Notice_all = "holy battleground of the pirates camp won!"
			--Notice ( Notice_all )
			SZ_Win_CountNum = SZ_Win_CountNum - 1
			--PK_JJHY_Winner = "navy camp"
			--PK_JJHY_Loser =  "pirate camp"
			...
			local Notice_all = "The Pirate Faction have Won the Sacred War!"
			Notice ( Notice_all )
		...
		if CheckMonsterDead ( haidaoSide_BaseRole ) == 1 then
			...
			--local Notice_all =  "holy battleground of the naval camp victory!"
			--Notice ( Notice_all )
			SZ_Win_CountNum = SZ_Win_CountNum - 1
			--PK_JJHY_Winner = "pirate camp"
			--PK_JJHY_Loser = "navy camp"
			...
			DealAllActivePlayerInMap(map_copy,"noside_Goto")
			DealAllActivePlayerInMap(map_copy,"Give_Three_QiYue")
			local Notice_all =  "The Navy Faction have Won the Sacred War!"
			Notice ( Notice_all )
		...
		if CheckMonsterDead ( haijunSide_LCRole ) == 1 then
			if haijunSide_LCNotice == 0 then
				MapCopyNotice ( map_copy ,"Navy Granary has been destroyed, defense of High Tower is greatly reduced")
		...
		if CheckMonsterDead ( haidaoSide_LCRole ) == 1 then
			if haidaoSide_LCNotice == 0 then 
				MapCopyNotice ( map_copy ,"Pirate Granary has been destroyed, defense of High Tower is greatly reduced")
		...
		if CheckMonsterDead ( haijunSide_DYKRole ) == 1 then
			if haijunSide_DYKNotice == 0 then
				MapCopyNotice ( map_copy ,"Navy Ammunition Warehouse has been destroyed, attack of High Tower is greatly reduced")
		...
		if CheckMonsterDead ( haidaoSide_DYKRole ) == 1 then
			if haidaoSide_DYKNotice == 0 then 
				MapCopyNotice ( map_copy ,"Pirates Ammunition Warehouse has been destroyed, attack of High Tower is greatly reduced")
		...
		if Count == math.floor ( Count ) then
			...
			local Notice_map = "Goddess will be spawning in " .. SZ_Win_CountNum .. " seconds!"
		...
		MapCopyNotice ( map_copy ,"Goddess spawned at Sacred War!")
...
--end
   ...
   Notice("Sacred War will close in 10 minutes!")
   end
   if second_five_seconds == 120 then
   Notice("Sacred War will close in 5 minutes!")
   end
   if second_five_seconds == 144 then
   Notice("Sacred War will close in 3 minutes!")
   end
   if second_five_seconds == 168 then
   Notice("Sacred War will close in 1 minute!")
   end
...
--
	--if winner == 1 then
		...
		--LG ( "PK_JJHY", "Lei Zhu victory") 
		--local Notice_all = "pirate camp won!"
		--Notice ( Notice_all )

	--elseif winner == 2 then
		--EndGuildChallenge ( RedSide_GuildID , BlueSide_GuildID , 1 )
		--LG ( "PK_JJHY", "Lei Zhu victory") 
		--local Notice_all = "navy camp won!"
		--Notice ( Notice_all )
	--else
		--local Notice_all = "fierce fighting between the two sides at 3 hours did not win, let us redouble our efforts for the next round!"
		--Notice ( Notice_all )
	--end
