...
function State_Tsqy_Rem ( role , statelv ) 
	...
	local hrecsa = (HrecSa(role) - hrecsa_dif ) * ATTR_RADIX
	if hrecsa < 0  then 
		--LG("Luaerror", "function State_Oper_Tsqy : 错误数据结果——", " Hrecsa = ", HrecSa(role) , " statelv = " , statelv , " hrecsa = ", hrecsa , "\n" ) 
		return 
	end 
	--LG("skill_tsqy" , " form_hrecsa = " , HrecSa(role) , "\n" ) 
	--LG("skill_tsqy", "hrecsa = ", hrecsa, "\n") 
...
function Skill_JLFT_BEGIN( role , sklv )
	...
	local State = {STATE_JLFT1,STATE_JLFT2,STATE_JLFT3,STATE_JLFT4,STATE_JLFT5,STATE_JLFT6,STATE_JLFT7,STATE_JLFT8}
	...
	if OtherStateLv > 0 then
		SkillUnable(role) 
		SystemNotice(role, "You are already possesed by fairy body!" ) 
		return
	end
...
function Skill_JLFT_End(ATKER, DEFER, sklv)
	...
	local item_elf_type	= GetItemType(item_elf)		--Pet Type
	local ItemID		= GetItemID(item_elf)
	...
	local FairyLv	= str + con + agi + dex + sta
	if item_elf_type == 59 and FairyLv > FAIRY_MAX_LV then
		SystemNotice(ATKER, "Internal error!")
		RemoveChaItem(ATKER,ItemID,2,2,1,2,1)
		--BTHLG("Crazy Fairy",""..GetChaDefaultName(ATKER).."\t"..ItemID.."\t"..FairyLv..": Str:"..str..", Con:"..con..", Agi:"..agi..", Dex:"..dex..", Sta:"..sta.."")
		return
	end
	local Fairy = {}
	Fairy[231] = STATE_JLFT1	--Fairy Of Luck
	Fairy[232] = STATE_JLFT2	--Fairy Of Strenght
	Fairy[233] = STATE_JLFT3	--Fairy of Constitution
	Fairy[234] = STATE_JLFT5	--Fairy Of Spirit
	Fairy[235] = STATE_JLFT4	--Fairy Of accuracy
	Fairy[236] = STATE_JLFT6	--Fairy Of Agility
	Fairy[237] = STATE_JLFT7	--Fairy Of evil
	Fairy[681] = STATE_JLFT8	--Mordo Junior
	Fairy[036] = STATE_JLFT8	--Angela Junior
	if Fairy[ItemID] ~= nil then
		AddState(ATKER, ATKER, Fairy[ItemID], statelv, statetime)
	else
		SystemNotice(ATKER, "Fairy Body failed!")
		return
	end
end 
...
function State_JLFT_Add(role, sklv)
	...
	if Get_Item_Type == 59 then 
		...
		if ItemID == 232 then
			star = FairyLv + math.floor(FairyLv * (sklv/10))
			SetCharaStatev(star, role, ATTR_STATEV_STR, "STATE_POSS")
		end
	
		if ItemID == 233 then
			star = FairyLv + math.floor(FairyLv * (sklv/10))
			SetCharaStatev(star, role, ATTR_STATEV_CON, "STATE_POSS")
		end

		if ItemID == 234 then
			star = FairyLv + math.floor(FairyLv * (sklv/10))
			SetCharaStatev(star, role, ATTR_STATEV_STA, "STATE_POSS")
		end

		if ItemID == 235 then
			star = FairyLv + math.floor(FairyLv * (sklv/10))
			SetCharaStatev(star, role, ATTR_STATEV_DEX, "STATE_POSS")
		end

		if ItemID ==236 then
			star = FairyLv + math.floor(FairyLv * (sklv/10))
			SetCharaStatev(star, role, ATTR_STATEV_AGI, "STATE_POSS")
		end
	
		local Fairy = {}
		Fairy[231]	= {BONUS = 0, PhyEff = 0}	--Fairy Of Luck
		Fairy[237]	= {BONUS = 0, PhyEff = 0}	--Fairy Of Evil
		Fairy[681]	= {BONUS = 0, PhyEff = 0}	--Mordo Junior
		Fairy[36]	= {BONUS = 0.2, PhyEff = 3}	--Angela Junior
		Fairy[7126]	= {BONUS = 0.2, PhyEff = 3}	--Angela Junior (Purple)
		if Fairy[ItemID] ~= nil then
			if str ~= nil and str ~= 0 then
				star	= str + math.floor(str * Fairy[ItemID].BONUS) + math.floor(FairyLv * (sklv/10))
				SetCharaStatev(star, role, ATTR_STATEV_STR, "STATE_POSS")
			end
			if con~= nil and con ~= 0 then
				star	= con + math.floor(con * Fairy[ItemID].BONUS) + math.floor(FairyLv * (sklv/10))
				SetCharaStatev(star, role, ATTR_STATEV_CON, "STATE_POSS")
			end
			if sta ~= nil and sta~= 0 then
				star	= sta + math.floor(sta * Fairy[ItemID].BONUS) + math.floor(FairyLv * (sklv/10))
				SetCharaStatev(star, role, ATTR_STATEV_STA, "STATE_POSS")
			end
			if dex ~= nil and dex ~= 0 then
				star	= dex + math.floor(dex * Fairy[ItemID].BONUS) + math.floor(FairyLv * (sklv/10))
				SetCharaStatev(star, role, ATTR_STATEV_DEX, "STATE_POSS")
			end
			if agi ~= nil and agi ~= 0 then
				star	= agi + math.floor(agi * Fairy[ItemID].BONUS) + math.floor(FairyLv * (sklv/10))
				SetCharaStatev(star, role, ATTR_STATEV_AGI, "STATE_POSS")
			end
			if Fairy[ItemID].PhyEff ~= 0 then
				star = Fairy[ItemID].PhyEff
				SetCharaStatev(star, role, ATTR_STATEV_PDEF, "STATE_POSS")
			end
		end
	end
	ALLExAttrSet(role)
...
function State_JLFT_Rem ( role , sklv )
	RemCharaStatev(0, role, ATTR_STATEV_STR, "STATE_POSS")
	RemCharaStatev(0, role, ATTR_STATEV_CON, "STATE_POSS")
	RemCharaStatev(0, role, ATTR_STATEV_STA, "STATE_POSS")
	RemCharaStatev(0, role, ATTR_STATEV_DEX, "STATE_POSS")
	RemCharaStatev(0, role, ATTR_STATEV_AGI, "STATE_POSS")
	RemCharaStatev(0, role, ATTR_STATEV_PDEF, "STATE_POSS")
	ALLExAttrSet(role)
	RefreshCha(role)
end

function State_None_Add ( role , sklv )
end
function State_None_Rem ( role , sklv )
end
				...
				if item_count == 1  then  
					...
					if Num_JZ == ID_ATK_Num   then
						...
						if Num_ATK_JZ==ID_Num  then
							...
							local checkteam = CheckRoleIsInJJCBS ( t[i] )   -------取角色是否在JJC内
							if checkteam == 0  then
								x = math.floor (x*0.01)
								y = math.floor (y*0.01)
								if map_name == "prisonisland" or map_name == "mingyun" or map_name == "guildwar" or map_name == "guildwar2" or map_name == "darkswamp" or map_name == "abandonedcity" or map_name == "abandonedcity2" or map_name == "abandonedcity3"  then
									...
									local checkteam2 = CheckRoleIsInJJCBS ( ATKER )   -------取角色是否在JJC内
...
function CheckRoleIsInJJCBS ( role ) 
	...
	if teamNum == 1 or teamNum == 3  then
		posIn= CheckPlayerIsInJJC(role,teamNum,teamlv ) -- IsPlayerInJJCMap(role) ：判断角色是否在竞技场地图的竞技区域，如果不在则返回0，在则返回1		
