...
function after_create_entry(entry) 
    ...
    local EntryName = "Aurora Area 2"
    SetMapEntryEventName( entry, EntryName )
    map_name, posx, posy, tmap_name = GetMapEntryPosInfo(entry)
    Notice("Aurora Area 2 Portal opened at ["..posx..","..posy.."] Winter Islands")

end
function after_destroy_entry_mjing2(entry)
    map_name, posx, posy, tmap_name = GetMapEntryPosInfo(entry) 
    Notice("Access to Aurora Area 2 entrance has disappeared!") 

end

function after_player_login_mjing2(entry, player_name)
    map_name, posx, posy, tmap_name = GetMapEntryPosInfo(entry)
    ChaNotice(player_name, "Winter Island  opened Portal to Aurora Area 2")

end

function check_can_enter_mjing2( role, copy_mgr )
...
function begin_enter_mjing2(role, copy_mgr) 
	...
	SystemNotice(role,"Entering [Aurora area 2]") 
