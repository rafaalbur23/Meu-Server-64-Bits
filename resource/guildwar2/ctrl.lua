...
--									--
--------------------------------------------------------------------------
print( "Loading Guildwar2 Ctrl.lua" )


--此文件中，凡是可能被多次执行的函数，函数名都要加上地图名前缀

function config(map)
    MapCanSavePos(map, guildwar2)	-- 1 = Allow players to save respaw // 0 = Do not allow players to save respaw // guildwar2 = Player will respaw in their base
...
local Cha = TurnToCha(role) 
  ...
  local map_copy =  GetChaMapCopy(role)
     ...
     if check_need_show == 1 then
	    ...
	    MapCopyNotice ( map_copy , "[A Navy member left Mini Sacred War]" )

         elseif GetChaGuildID(role) > 100 and GetChaGuildID(role) <= 200 then
            count_haidao = count_haidao - 1
	    MapCopyNotice ( map_copy , "[A Pirate member left Mini Sacred War]" )
        end
    end
      ...
      local closetime = GUILDWARCLOSETIME2
		for i = 1 , GUILDNOTICE2 , 1 do
			if closetime ==GUILDCLOSESHOW2[i] then
				local Notice_all = "Mini Sacred War will close in " .. closetime .. " seconds! Check announcement for more details. Enjoy!"
...
--		local GuildName_Blue = GetGuildName ( BlueSide_GuildID )
		...
		if CheckMonsterDead ( di_haijunSide_BaseRole ) == 1 then
		   if di_haidaoSide_JDNotice == 0 then
			...
			local Notice_all = "The Pirate Faction have Won the Mini Sacred War!"
			Notice ( Notice_all )
		...
		if CheckMonsterDead ( di_haidaoSide_BaseRole ) == 1 then
			...
			--DealAllActivePlayerInMap(map_copy,"noside_Goto2")
			DealAllActivePlayerInMap(map_copy,"Give_Three_QiYue2")
			local Notice_all = "The Navy Faction have Won the Mini Sacred War!"
			Notice ( Notice_all )
		...
		if Count == math.floor ( Count ) then
			...
			local Notice_map = "Goddess will be spawning in "..SZ_Win_CountNum2.." seconds!"
		...
		MapCopyNotice ( map_copy ,"Goddess spawned at Mini Sacred War!")
...
--end
   ...
   Notice("Mini Sacred War will close in 10 minutes!")
   end
   if second_five_seconds == 120 then
   Notice("Mini Sacred War will close in 5 minutes!")
   end
   if second_five_seconds == 144 then
   Notice("Mini Sacred War will close in 3 minutes!")
   end
   if second_five_seconds == 168 then
   Notice("Mini Sacred War will close in 1 minute!")
   end
