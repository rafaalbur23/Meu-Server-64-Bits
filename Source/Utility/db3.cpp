#define SQL_NOUNICODEMAP // Required to prevent conflicts with macros 
...
bool CSQLDatabase::SafeSQL(char const* sql, unsigned short num, ...)
{
	...
	_rc = SQLSetStmtAttr(_hstmt, SQL_ATTR_QUERY_TIMEOUT, SQLPOINTER(timeout), SQL_IS_UINTEGER);
	if(DBFAIL(_rc))
	{
		ThrowError();
	}
	...
	unsigned short numberOfParameters = va_arg(arg_list, unsigned short);
	SQLLEN	   data_len			  = SQL_NTS;

	_rc = SQLPrepare(_hstmt, (SQLCHAR*)sql, SQL_NTS);
	if(DBFAIL(_rc))
	{
		ThrowError();
	} else
	{
		...
		for(int i = 1; i <= numberOfParameters; i++)
		{
			...
			// size_t dataLen
			size_t		param_length  = va_arg(arg_list, size_t);
			const char* param_pointer = va_arg(arg_list, const char*);

			_rc = SQLBindParameter(_hstmt, i, SQL_PARAM_INPUT, SQL_C_BINARY, SQL_BINARY, param_length, 0,
								   (SQLPOINTER)param_pointer, 0, &data_len);
...
bool CSQLRecordset::SafeSQL(char const* sql, unsigned short num, ...)
{
	_rc = ::SQLAllocHandle(SQL_HANDLE_STMT, _hdbc, &_hstmt);
	if(!(_rc == SQL_SUCCESS || _rc == SQL_SUCCESS_WITH_INFO))
		ThrowError(SQL_HANDLE_DBC);
