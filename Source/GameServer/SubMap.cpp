...
{T_B
	...
	// 状态单元
	m_pCStateCell = new(std::nothrow) CStateCell**[GetStateCellLin()];
	if (!m_pCStateCell)
	  throw excpMem(RES_STRING(GM_SUBMAP_CPP_00001));
	for (short i = 0; i < GetStateCellLin(); i++)
	{
		m_pCStateCell[i] = new(std::nothrow) CStateCell*[GetStateCellCol()];
		if (!m_pCStateCell[i])
		  throw excpMem(RES_STRING(GM_SUBMAP_CPP_00001));
	...
	// 视野单元
	m_pCEyeshotCell = new(std::nothrow) CEyeshotCell*[GetEyeshotCellLin()];
	if (!m_pCEyeshotCell)
		throw excpMem(RES_STRING(GM_SUBMAP_CPP_00002));
	short	sECellSX, sECellEX, sECellSY, sECellEY;
	short	sSCellNum;
	for (short i = 0; i < GetEyeshotCellLin(); i++)
	{
		m_pCEyeshotCell[i] = new(std::nothrow) CEyeshotCell[GetEyeshotCellCol()];
	...
	CParser::dofile(g_pLuaState, GetResPath(m_pCMapRes->m_szMonsterCofFile));
	m_bIsRun = false;
	m_sCopyNO = sCopyNO;
...
{T_B
	...
	{
	throw excpMem(RES_STRING(GM_SUBMAP_CPP_00003));
		return FALSE;
	}
...
{T_B
	...
	else // 分配新的载体
	{
		...
		if (!pSCarrier)
		{
		  throw excpMem(RES_STRING(GM_SUBMAP_CPP_00006));
		}
	}
