...
{	
	...
	{T_B
		...
		int nStatus = CParser::docall(g_pLuaState, 1, 0);
		if( nStatus )
		{
	...
	{T_B
		...
		if( pszData )
		{
			...
			// 校验船名是否正确
			if(!common::conformity::character::name::is_valid(pszData, (USHORT)nLen) ||
			   !CTextFilter::IsLegalText(CTextFilter::NAME_TABLE, pszData))
			{
			  pBoat->Free();
			  owner.SetBoat(NULL);
