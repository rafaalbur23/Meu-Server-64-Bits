...
bool CChaSpawn::Init(char *szSpawnTable, long lRegionNum)
{
	...
	if (lRegionNum <= 0)
	  ...
	  throw std::logic_error(RES_STRING(GM_ENTITYSPAWN_CPP_00001));
	...
	m_pCMonRefRecordSet = new(std::nothrow) CMonRefRecordSet(0, lRegionNum);
	if (!m_pCMonRefRecordSet)
	  throw excpMem(RES_STRING(GM_ENTITYSPAWN_CPP_00002));
	if (!LoadTable(m_pCMonRefRecordSet, m_szSpawnTable))
		return false;

	m_lRegionNum = lRegionNum;
	m_pSMonInfo = new(std::nothrow) SMonInfo[m_lRegionNum];
	if (!m_pSMonInfo)
	  throw excpMem(RES_STRING(GM_ENTITYSPAWN_CPP_00003));
...
bool CMapSwitchEntitySpawn::Init(char *szSpawnTable, long lRecordNum)
{
	...
	if(lRecordNum <= 0)
	  throw std::logic_error(RES_STRING(GM_ENTITYSPAWN_CPP_00005));
	//strcpy(m_szSpawnTable, szSpawnTable);
	strncpy_s( m_szSpawnTable, sizeof(m_szSpawnTable), szSpawnTable, _TRUNCATE );
	m_lRecordNum = lRecordNum;

	m_pCSwitchMapRecSet = new(std::nothrow) CSwitchMapRecordSet(0, lRecordNum);
	if (!m_pCSwitchMapRecSet)
	  throw excpMem(RES_STRING(GM_ENTITYSPAWN_CPP_00006));
...
bool CNpcSpawn::Init( char *szSpawnTable, long lRecordNum )
{
	...
	m_pNpcRecordSet = new(std::nothrow) CNpcRecordSet(0, lRecordNum);
	if( !m_pNpcRecordSet )
	{
	  throw excpMem(RES_STRING(GM_ENTITYSPAWN_CPP_00008));
	}
