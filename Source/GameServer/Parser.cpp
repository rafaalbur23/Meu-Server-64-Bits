...
{T_B
	...
	LETimer t; t.Begin();
	//lua_pushcfunction(m_pSLua, lua_traceback);
	...
	//int nState = lua_pcall(m_pSLua, nParamNum, LUA_MULTRET, 0 /* lua_gettop(m_pSLua) - (nParamNum + 1)*/);
	...
	//lua_remove(m_pSLua, lua_gettop(m_pSLua) - (nParamNum + 1));
	int	nRet = 1;
	int i = 0;
	for (; i < nRetNum; i++)
	{
