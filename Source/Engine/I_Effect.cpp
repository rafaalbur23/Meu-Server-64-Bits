...
int I_Effect::BoundingRes(CLEResManger	*m_CResMagr, char* pszParentName)
{
	...
	if(_eEffectType == EFFECT_FRAMETEX)
	{
		for (WORD n = 0; n < m_CTexFrame.m_wTexCount; ++n)
		{
			m_CTexFrame.m_vecTexs[n] = 
				m_CResMagr->GetTextureByNamedr(m_CTexFrame.m_vecTexName[n]);
			if(!m_CTexFrame.m_vecTexs[n])
			{
				char pszMsg[64];
				_snprintf_s(pszMsg,64,_TRUNCATE, "特效文件[%s]加载帖图[%s]时出错",
	...
	else
	{
		m_CTextruelist.m_pTex = 
			m_CResMagr->GetTextureByNamedr(m_CTextruelist.m_vecTexName);
	...
	if (!_bBillBoard && IsTobMesh(m_strModelName))
	{
		if(!m_pCModel)
		{
			...
			{
				char pszMsg[64];
				_snprintf_s(pszMsg,64, _TRUNCATE,"特效文件[%s]缺少可变模型[%s]",
	...
	else
	{
		...
		{
			char pszMsg[64];
			_snprintf_s(pszMsg,64,_TRUNCATE, "特效文件[%s]加载模型[%s]时出错",
				pszParentName,m_CTextruelist.m_vecTexName.c_str());
			LG("ERROR","msg%s",pszMsg);
			return 2;
		}
	...
	//{
	//		//char pszn[128];
	//		//_snprintf_s(pszn, "texture\\effect\\%s",m_CTextruelist.m_vecTexName.c_str());
...
bool	CEffectModel::CreateTriangle()
{
	...
	//// mesh
	//// test lock method
	//drFVFStruct_XyzDiffuse* buf;
	//drILockableStreamVB* lsvb = g_mesh0->GetLockableStreamVB();
...
bool	CEffectModel::CreateCone(int nSeg,float fHei,float fRadius)
{
	...
	int nCurrentSegment;
	//drVector3* pVertex = mi.vertex_seq;
	//drVector2* pCoord = mi.texcoord0_seq;
	int idx = 0;
...
bool	CEffectModel::CreateShadeModel(WORD wVerNum , WORD wFaceNum,int iGridCrossNum,bool usesoft)
{
	...
	{
		mi.vercol_seq[n] = 0xffffffff;
		mi.vertex_seq[n] = drVector3(0,0,0);//!必须全部为零
...
#ifdef USE_RENDER 
bool	CEffectFont::CreateEffectFont(LERender*  pDev,
									  CLEResManger*pCResMagr,int iTexID,D3DXCOLOR dwColor, bool bUseBack,bool bmain)
#else
...
#endif
{
	...
	{
		//if(_iTextureID == 6)
		//	_snprintf_s(psName,"%s.tga",str[_iTextureID].c_str());
		//else
	...
	while (!feof(fFile))
	{
		...
		while (!feof(fFile))
		{
			...
			if (tcsLine[lIndex] == _TEXT('\t')) // tab键
			{
				...
				if (_ttoi(tcsTemp) < 1)
				{
					MessageBox(NULL, _TEXT("please update .\\scripts\\CharacterAction.tx"), _TEXT("错误"), 0);
					goto end;
				}
				//
