...
Archive数据流的LEDataStream的派生类，除非默认提供的可以满足要求
@note
...
public:
	...
	*/
	...
	@param freeOnClose 如果为true，当流释放时，内存将会被释放.
	*/
	LEMemoryDataStream(const String& name, void* pMem, size_t size, 
	...
	*/
	...
	@param freeOnClose 如果为true，当流释放时，内存将会被释放.
	*/
	LEMemoryDataStream(const String& name, LEDataStream& sourceStream, 
	...
	@param freeOnClose 如果为true，当流释放时，内存将会被释放.
	*/
	LEMemoryDataStream(const String& name, const LEDataStreamPtr& sourceStream, 
		bool freeOnClose = true);
	/** 创建一个空的流.
	@param size 流的大小
	@param freeOnClose 如果为true，当流释放时，内存将会被释放.
