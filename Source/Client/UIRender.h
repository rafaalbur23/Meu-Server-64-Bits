...
class UIRender
{
  ...
  void RenderSprite(LPTEXTURE tex, RECT* rc, VECTOR2* vscale, VECTOR2* vdest, DWORD dwColor, FLOAT rotation = 0.0f);
  void RenderTextureRect(int nX, int nY, LETexRect* pRect, FLOAT rotation = 0.0f);
...
inline void UIRender::RenderSprite(LPTEXTURE tex, RECT* rc, VECTOR2* vscale, VECTOR2* vdest, DWORD dwColor, FLOAT rotation)
{
  ...
  if (rotation != 0.0f) {
	rotationCenter.x = (rc->left + rc->right) / 2.f;
	rotationCenter.y = (rc->top + rc->bottom) / 2.f;
  }
  if(_p2DSprite) _p2DSprite->Draw(tex, rc, vscale, &rotationCenter, rotation, vdest, dwColor);
}
