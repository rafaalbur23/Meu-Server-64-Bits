...
void CItemCommand::SaleRender(int x, int y, int nWidth, int nHeight)
{
	...
	if (_ItemData.sID == 10841 || _ItemData.sID == 15047 || _ItemData.sID == 15048 || _ItemData.sID == 15049 || _ItemData.sID == 15050 || _ItemData.sID == 15051 || _ItemData.sID == 15052 || _ItemData.sID == 15053 || _ItemData.sID == 15054 || _ItemData.sID == 15055 || _ItemData.sID == 15056) {
...

 {
	...
	else if (sType < 29 || sType >= 81 && sType <= 83 && sType != 12 && sType != 13 && sType != 17 && sType != 18 && sType != 19 /*&& sType != 20*/ && sType != 21) { // rendering forge level on icons
...
void CItemCommand::Render(int x, int y)
{
...

 {
	...
	if (sType < 29 || sType >= 81 && sType <= 83 && sType != 12 && sType != 13 && sType != 17 && sType != 18 && sType != 19 /*&& sType != 20*/ && sType != 21) {
		SItemForge& Forge = GetForgeInfo();
		if (Forge.IsForge) {
			if (Forge.nLevel > 0) {
				sprintf(buf, "+%d", Forge.nLevel);
...
bool CItemCommand::UseCommand()
{
  ...
  if(pOwn)
  {
	...
	if(_pItem->sType == enumItemTypePetFodder || _pItem->sType == enumItemTypePetSock)
	{
	  ...
	  return false;
	} 		if (_pItem->lID == 10841 || _pItem->lID == 15045 || _pItem->sType == 66 || (864 <= _pItem->lID && _pItem->lID <= 866)) // ìí?ó?àì?×′ì?  add by Philip.Wu  2006-06-20
		{
			CActor* pActor = CGameScene::GetMainCha()->GetActor();
			...
			pActor->SwitchState(pState);
			return false;
		} else if (GetItemInfo()->sType == 71) // ???üμà??ê1ó?
	{
	  CCharacter* pCha = CGameScene::GetMainCha();
	  if(!pCha) return false;
