...
void CStartMgr::PopMenu(CCharacter* pCha)
{
  ...
  if ( mainMouseRight && pCha && pCha->IsValid() && !pCha->IsHide() && (pCha->IsPlayer() || pCha->IsMonster()) ) // && (pCha->IsPlayer() || pCha->IsMonster())
	...
	for(int i = 0; i < nCount; i++)
	{
	  ...
	  if(_stricmp(pItem->GetString(), "View Info") == 0){
				if(pCha->IsMonster()) {
			...
			if(_stricmp(pItem->GetString(), "View Drops") == 0){
				if(pCha->IsMonster()) {
					pItem->SetIsHide(false);
					pItem->SetIsEnabled(true);
	...
	{	// ????????????????鹦??  Add by ning.yan  20080715 Begin
		//if( stricmp( MapName,"starena1") == 0 || stricmp( MapName,"starena2") == 0 || stricmp( MapName,"starena3") == 0 )
		//	pItem->SetIsEnabled( false );
		//// End
		//else
...
void CStartMgr::SetDropsInfo (CCharacter* pTargetCha) {
		...
		CItem			*content;
		CCharacter* pMain = CGameScene::GetMainCha();
		int STATELV = pMain->GetStateMgr()->GetStateLv( 49 );
		float GLOBAL_RATE = 1.0;
		float PetRate = 1.0;
		int MordoJr = pMain->GetStateMgr()->GetStateLv( 174 );
		
		SItemGrid item = pMain->GetPart().SLink[enumEQUIP_FAIRY];
		int PetLevel = item.GetAttr(ITEMATTR_VAL_STR)
		...
		if (STATELV > 0)
		{
			if(STATELV == 1)// 2.0 amp
			{
				GLOBAL_RATE = GLOBAL_RATE * 2;
			}
			else if(STATELV == 2) //2.5 amp
			{	
				GLOBAL_RATE = GLOBAL_RATE * 3;
			}
			else if(STATELV == 3) //3.0 amp
			{	
				GLOBAL_RATE = GLOBAL_RATE * 3;
			}
		...
		for(int i = 0; i < max; i++){
			...
			float per =  GLOBAL_RATE * PetRate * (10000/float(vect[i][1]));
...
void CStartMgr::_evtOnClose( CForm* pForm, bool& IsClose )  // ??????????
{
	pTarget = NULL;
	pLastTarget = NULL;
}