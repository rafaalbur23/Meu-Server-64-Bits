...
{
  public:
  CLimit(T max, T min = 0) :
	  _max(max), _min(min) { _cur = min; }
  CLimit() :
	  _max(0), _min(0), _cur(0) {}
...
class CSizeArray
{
  public:
  CSizeArray() :
	  _pChange(NULL), _nCount(0){};

  CSizeArray(const CSizeArray& rhs) :
	  _pChange(NULL), _nCount(0)
  {
...
{
  public:
  CTimeWork(DWORD sleep = 1000) :
	  dwTime(0), dwSleep(sleep) {}
