...
{
  public:
  CGraph(char* file, int w, int h, int sx, int sy, int maxframe) :
	  nTag(0)
  {
  ...
  CGraph(const CGraph& rhs) :
	  CItemObj(rhs), _pImage(new CGuiPic(*rhs._pImage)), nTag(rhs.nTag) {}
  ...
  CGraph() :
	  nTag(0) { _pImage = new CGuiPic(NULL, 1); }
  ~CGraph() { SAFE_DELETE(_pImage); } // UI当机处理 //delete _pImage;					}
  ITEM_CLONE(CGraph)

  CGraph(unsigned int frame) :
	  nTag(0) { _pImage = new CGuiPic(NULL, frame); }
