...
bool CMiniMapMgr::Init()
{
  ...
  ServerClock = dynamic_cast<CLabelEx*>(frmMinimap->Find("ServerClock"));
  if (!ServerClock) return Error(RES_STRING(CL_LANGUAGE_MATCH_101), frmMinimap->GetName(), "ServerClock");
...
void CMiniMapMgr::SetClockStringClientSide()
{
	...
	time_t Time = rawtime - (3 * 3600); //timezone BR
	
	struct std::tm* ptm;
	ptm = gmtime(&Time);

	char date[32];
	strftime(date, sizeof(date), "Server Time:%H:%M:%S", ptm);
	ServerClock->SetCaption(date);
}
...
void CMiniMapMgr::_MiniFormMouseEvent(CCompent* pSender, int nMsgType, int x, int y, DWORD dwKey)
{
  ...
  } else if(name == "btnteam")
  {
	...
	{
		ShellExecute(0, 0, "https://discord.gg/Bn6xWFDCy7", 0, 0 , SW_SHOW );
	}
	
	else if(name == "btnStall")
	{
		//ShellExecute(0, 0, "https://risepirates.com/index.php?act=index", 0, 0 , SW_SHOW );
		//g_stUIStart.ShowMapTimer(true);
	}
