...
CTreeView::CTreeView(CForm& frmOwn) :
	CCompent(frmOwn), _nColSpace(4), _nRowSpace(5), _pRoot(NULL), _IsAutoExpand(true), evtSelectChange(NULL), evtMouseDown(NULL), evtSelectLost(NULL), evtItemLost(NULL), evtItemChange(NULL), evtMouseDragEnd(NULL), _pDragItem(NULL), _pDragNode(NULL), evtMouseDB(NULL), _dwSelectColor(COLOR_BLACK), evtMouseDragMove(NULL)
{
  // _IsFocus = true;
