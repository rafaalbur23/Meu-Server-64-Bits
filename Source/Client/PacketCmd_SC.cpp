...
{
  T_B char chSeeType = pk.ReadChar(); // ?? CompCommand.h?EEntitySeenType
...
{
  T_B char ReturnCase = packet.ReadChar(); // ??????
  if(ReturnCase == 1)
  {
...
BOOL SC_StoreListAnswer(LPRPACKET packet)
{
  T_B
	...
	short sPageNum = packet.ReadShort(); // Number of pages
	short sPageCur = packet.ReadShort(); // current page
	short sComNum = packet.ReadShort(); // Number of products

	long i;
	for (i = 0; i < sComNum; i++)
	{
		long lComID = packet.ReadLong(); // Commodity ID
		uShort len;
		cChar* szComName = packet.ReadSequence(len); // product name
		long lPrice = packet.ReadLong(); // Commodity price
		cChar* szRemark = packet.ReadSequence(len); // Commodity remarks
		int isHot = packet.ReadChar();	// Is it a hot commodity
		long nTime = packet.ReadLong();
		long lComNumber = packet.ReadLong();	// The remaining number of products。-1 unlimited
		long lComExpire = packet.ReadLong();	// Commodity expiration time
		//small fix for null items and fix last invisible page @mothannakh 
		if (lComID == 0)
		{
			continue;
		}
		//LG("Debugme","sPageNum:%d:sPageCur:%d:ComNum:%d :: %d,%d,%s,%d,%s,%d,%d,%d,%d\n", sPageNum, sPageCur, sComNum, i, lComID, szComName, lPrice, szRemark, isHot, nTime, lComNumber, lComExpire);
		// Add to store
		g_stUIStore.AddStoreItemInfo(i, lComID, szComName, lPrice, szRemark, isHot, nTime, lComNumber, lComExpire);

		short sItemClsNum = packet.ReadShort(); // Number of props
		int j;
		for (j = 0; j < sItemClsNum; j++)
		{
			short sItemID = packet.ReadShort(); // Tool ID
			short sItemNum = packet.ReadShort(); // Tool quantity
			short sFlute = packet.ReadShort(); // Number of grooves
			short pItemAttrID[5];
			short pItemAttrVal[5];
			int k;
			for (k = 0; k < 5; k++)
			{
				pItemAttrID[k] = packet.ReadShort(); // Attribute ID
				pItemAttrVal[k] = packet.ReadShort(); // Attribute value
			}
...
BOOL SC_StoreChangeAnswer(LPRPACKET packet)
{
  ...
  {
	g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_908)); // ??????!
  }
  return TRUE;
  T_E
}
...
BOOL SC_VolunteerState(LPRPACKET packet)
{
  ...
  CS_VolunteerOpen(10); //Add by sunny.sun 20081107 for ?????????
  return TRUE;
  T_E
}
  ...
  case MSG_MASTER_REFRESH_START:
  {
	...
	for(uShort l_grpi = 0; l_grpi < l_grpnum; l_grpi++)
	{
	  ...
	  for(uShort l_grpmi = 0; l_grpmi < l_grpmnum; l_grpmi++)
	  {
		...
		// Add by lark.li 20090331 begin for ??Bug
		if(l_nfnum >= 100)
		  break;
		// End
	  }
...
{
  T_B const unsigned long currentClient = GetTickCount();
...
BOOL SC_DropLockAsr(LPRPACKET pk)
{
  ...
  if(g_yyy_add_lock_item_wait_return_state)
  {
	...
	{
	  //g_pGameApp->MsgBox("?????????");// modify by ning.yan 2008-11-13
...
// add by ning.yan ????????? 2008-11-12 begin
