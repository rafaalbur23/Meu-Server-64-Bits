...
class CGuiPic
{
  ...
  void Render(int x, int y, float rotation = 0.0f);
...
inline void CGuiPic::Render(int x, int y, float rotation)
{
  GetRender().RenderTextureRect(x, y, &_pImage[_frame], rotation);
}
...
{
  GetRender().RenderTextureRect(x, y, &_pImage[_frame], (DWORD)(bAplha << 24) | 0x00FFFFFF);
}
