...
CGuiData::CGuiData() :
	_dwID(0), _nLeft(0), _nTop(0), _nWidth(0), _nHeight(0), _nX1(0), _nX2(0), _nY1(0), _nY2(0), _bShow(true), _bEnabled(true), _pParent(NULL), _pDrag(NULL), _IsMouseIn(false), nTag(0), _pVoid(NULL)
{
...
CGuiData::CGuiData(const CGuiData& rhs) :
	_pParent(rhs._pParent), _nWidth(rhs._nWidth), _nHeight(rhs._nHeight), _nLeft(rhs._nLeft), _nTop(rhs._nTop), _nX1(rhs._nX1), _nX2(rhs._nX2), _nY1(rhs._nY1), _nY2(rhs._nY2), _bShow(rhs._bShow), _bEnabled(rhs._bEnabled), _strName(rhs._strName), nTag(rhs.nTag), _pVoid(rhs._pVoid), _pDrag(NULL), _IsMouseIn(false), _strHint(rhs._strHint)
{
  _AllGui.push_back(this);
  _dwID = (DWORD)(_AllGui.size());
...
CDrag::CDrag() :
	evtMouseDragBegin(NULL), evtMouseDragMove(NULL), evtMouseDragEnd(NULL), _eState(stNone), _nYareLen(2), _IsMove(true), _IsUseGrid(true), _crDragIn(CCursor::stEnd), _crDrag(CCursor::stDrag)
{
}
