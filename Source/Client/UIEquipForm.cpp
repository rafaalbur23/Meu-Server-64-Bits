...
{
    // Verificar se a tecla é um atalho para mudar a hotbar
    if (key == VK_OEM_4 && ::GetAsyncKeyState(VK_CONTROL)) // Ctrl + [
    {
        // Troca para a hotbar anterior (se necessário)
        _ActiveFast(_nFastCur - 1);
        return true;
    }
    else if (key == VK_F12 || (key == VK_OEM_6 && ::GetAsyncKeyState(VK_CONTROL))) // Ctrl + ]
    {
        // Troca para a hotbar seguinte (se necessário)
        _ActiveFast(_nFastCur + 1);
        return true;
    }

    // Sempre ative as teclas numéricas (0-9)
    if (key >= '0' && key <= '9')
    {
        key = key - '0' + 11; // Mapeia 0 para 11, 1 para 12, ..., 9 para 20
    }
    // Verificar se a tecla é uma tecla de fun??o (F1 a F11)
    else if (key >= VK_F1 && key <= VK_F11)
    {
        key -= VK_F1; // Mapeia F1 para 0, F11 para 10
    }
    else
    {
    ...
    // Verificar se o hotkey está habilitado
    if (!CFormMgr::s_Mgr.GetEnableHotKey()) return false; // 禁用热键

    // Ajustar o índice baseado na hotbar ativa
    key = (_nFastCur * MAX_FAST_COL) + key; // MAX_FAST_COL deve ser 11 para F1-F11 + 10 para 0-9

    // Executar o comando se estiver disponível
