...
CCombo::CCombo(CForm& frmOwn) :
	CCompent(frmOwn), _pEdit(new CEdit(frmOwn)), _pList(new CList(frmOwn)), _pButton(new CTextButton(frmOwn)), _eListStyle(lsDown), evtSelectChange(NULL), evtMouseClick(NULL)
{
  _IsFocus = false;
  _SetSelf();
}
