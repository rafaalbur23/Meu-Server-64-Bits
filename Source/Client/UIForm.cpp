...
CForm::CForm() :
	CGuiData(), _isModal(false), _nActiveCompentID(0), _modalResult(mrNone), _nOffWidth(0), _nOffHeight(0), evtShow(NULL), evtHide(NULL), evtActive(NULL), evtLost(NULL), evtMouseDown(NULL), evtHotkeyHandler(NULL), evtEntrustMouseEvent(NULL), evtKeyDown(NULL), evtKeyChar(NULL), _cHotKey(0), _formStyle(fsNone), _IsEscClose(true), evtMouseDragBegin(NULL), evtMouseDragEnd(NULL), evtOnSetScreen(NULL), _pPointer(NULL), _pPopMenu(NULL), evtClose(NULL), _pEnterButton(NULL), evtBeforeShow(NULL), evtEscClose(NULL), _isInMainForm(false)
{
