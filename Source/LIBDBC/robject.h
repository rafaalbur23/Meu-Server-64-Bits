...
protected:
	...
	{
		if(sync&&!_m_SyncMutex.Create(false)) throw excpSync("同步对象建立错误.");
...
{
public:
	rptr()					:_m_objptr(0)				{if(sync && !(_m_SyncMutex.Create(false))) throw excpSync("互斥体建立错误");};
	rptr(const rptr &ptr)	:_m_objptr(ptr._m_objptr)	{if(sync && !(_m_SyncMutex.Create(false))) throw excpSync("互斥体建立错误");if(_m_objptr)_m_objptr->adopt();};
	rptr(T *objptr)			:_m_objptr(objptr)			{if(sync && !(_m_SyncMutex.Create(false))) throw excpSync("互斥体建立错误");if(_m_objptr)_m_objptr->adopt();};
	~rptr()
	{
		lock();
		try{
